...
contract ERC20TokenExchange{
    ...
    */
    ...
    function getErc20Offers(uint exchangeId) external view returns(ERC20Offer[] memory){
        uint count = 0;
        for(uint i = 1; i <= _erc20OfferCount; i++) {
            if(_erc20Offers[i].exchangeId == exchangeId) {
                count++;
            }
        }

        ERC20Offer[] memory offers = new ERC20Offer[](count);
        uint temp = 0;
        for(uint i = 1; i <= _erc20OfferCount; i++) {
            if(_erc20Offers[i].exchangeId == exchangeId) {
                offers[temp] = _erc20Offers[i];
                temp++;
            }
        }
